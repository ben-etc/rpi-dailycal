...
def convert_bitmap(rgb_image, screencolor, date_location="detect", date_coords=(0,0)):
    ...
    if date_location == "detect":
        secret_pixel = (-1,-1) # This will be used to figure out where the calendar will be placed on the image
        ...
        for y in range(imgheight):
            for x in range(imgwidth):
                if mode == "RGB":
                    ...
                    inverse_alpha = 255 - a
                    r, g, b = (r + inverse_alpha), (g + inverse_alpha), (b + inverse_alpha)
                ...
                # Math for yellow screens
                    ...
                    yellow_avg = int(((r-b) + (g-b)) / 2) # Average the red and green channels to try to get a yellow value
...
def merge_bitmaps(black_bitmap, color_bitmap, screencolor="red"):
    ...
    outputImage = Image.new("RGB", (color_bitmap.width, color_bitmap.height), white)
...
def add_dateboxes(screencolor, black_bitmap, color_bitmap, secret_pixel, border=True, margin=5, fontname="LeagueSpartan-Bold.otf"):
    ...
    width = max( (margin * 2 + font.getsize(day_of_week)[0]), (margin + indent + font.getsize(formatted_date)[0]))
    ...
    if screencolor != "grayscale":
        ...
        color_draw.text((indent - 2, newline + 2), formatted_date, fill=0, font=font) # Shadow
        color_draw.text((indent, newline), formatted_date, fill = 1, font = font)
        if border == True:
            black_draw.rectangle((0,0, black_date.width, black_date.height), fill=1, outline=0, width=2)
        black_draw.text((margin - 2, margin + 2), day_of_week, fill=0, font=font) # Shadow
